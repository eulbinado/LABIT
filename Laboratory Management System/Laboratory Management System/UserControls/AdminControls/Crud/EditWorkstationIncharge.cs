...
namespace Laboratory_Management_System.UserControls.AdminControls.Crud
{
    ...
    WHERE workstation_id = '{workstationId}'";
        ...
        private void PopulateSystemUnitDropdown(string selectedUnitId = null)
        {
            ...
            // Retrieve the current operating system of the system unit
            string currentOperatingSystemQuery = $@"
SELECT su.operating_system 
FROM workstations w
LEFT JOIN system_units su ON w.system_unit = su.system_unit_id
WHERE w.workstation_id = '{workstationId}'";

            DataTable osData = dbHelper.GetData(currentOperatingSystemQuery);
            if (osData.Rows.Count == 0)
            {
                MessageBox.Show("Unable to determine the operating system of the current system unit.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            string currentOperatingSystem = osData.Rows[0]["operating_system"].ToString();

            string systemUnitFilter;
            if (currentOperatingSystem == "Windows")
            {
                systemUnitFilter = "operating_system IN ('Windows', 'Linux')";
            }
            else if (currentOperatingSystem == "MAC")
            {
                systemUnitFilter = "operating_system = 'MAC'";
            }
            else
            {
                systemUnitFilter = $"operating_system = '{currentOperatingSystem}'";
            }
