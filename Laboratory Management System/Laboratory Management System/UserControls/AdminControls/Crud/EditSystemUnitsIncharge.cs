...
namespace Laboratory_Management_System.UserControls.AdminControls.Crud
{
    public partial class EditSystemUnitsIncharge : Form
    {
        ...
        private void ToggleControlsForMac(bool isMac)
        {
            operatingSystem.Enabled = !isMac;
            motherboard.Enabled = !isMac;
            cpu.Enabled = !isMac;
            ram.Enabled = !isMac;
            gpu.Enabled = !isMac;
            psu.Enabled = !isMac;
            system_unit_case.Enabled = !isMac;
            storage.Enabled = !isMac;
        ...
        WHERE system_unit_id = '{systemUnitId}'";
            ...
            if (systemUnitData.Rows.Count > 0)
            {
                ...
                bool isMac = row["operating_system"].ToString().Equals("MAC", StringComparison.OrdinalIgnoreCase);
                ToggleControlsForMac(isMac);
            }
            else
            {
        ...
        {
            bool isMac = operatingSystem.SelectedItem.ToString().Equals("MAC", StringComparison.OrdinalIgnoreCase);
            ToggleControlsForMac(isMac);
        }
